...
$ArtifactModPath = "$PublishPath/archive" -replace "\\", "/" 
...
# Copy licenses
New-Item -ItemType Directory -Path $ArtifactModPathBackslash\$ModFolder\licenses -Force | Out-Null
Write-Host "Copy license files to artifacts mod folder." -ForegroundColor Yellow
xcopy $PSScriptRoot\Installer\licenses\* $ArtifactModPathBackslash\$ModFolder\licenses /E /Q /S /Y
...
if ($PSBoundParameters.ContainsKey('AdditionalModFolders')) {
  $NsisArguments += "/DADDITIONAL_MOD_FOLDERS_COUNT=$($AdditionalModFolders.Count)"
  for ($i = 0; $i -lt $AdditionalModFolders.Count; $i++) {
    $NsisArguments += "/DMOD_FOLDER$($i + 2)=""$($AdditionalModFolders[$i])"""
  }
}
